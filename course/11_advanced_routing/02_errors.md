...
déjà vu [comment afficher des pages d'erreurs](../01_sveltekit_basics/11_error_pages) lorsqu'une
page est introuvable (erreur 404), ou lorsqu'une erreur inattendue se produit sur le serveur (erreur
500).

Mais vous pouvez aussi vous-même décider que quelque chose n'est pas correct, et afficher une page
d'erreur.

## Erreurs attendues

Si vous décidez que quelque chose ne va pas sur le serveur, vous pouvez utiliser l'utilitaire
`error` du module `@sveltejs/kit`. Vous pouvez utiliser `error` dans les fonctions `load`, les
...
Vous devez fournir 2 choses à `error` :
...
> un simple `throw`, mais utiliser `error` est plus pratique avec SvelteKit.
...
_Dans la fonction load de la page /pokedex/[id]_

- renvoyer une erreur `404` si on ne trouve pas de Pokémon correspondant à cet `id`, avec un message
...
[Plus d'infos sur ce chapitre](https://svelte.dev/docs/kit/load#errors)
